...
async function listAccessibleAccounts(cfg) {
    ...
    const accounts = await Promise.all(resourceNames.map(async (resourceName) => {
...
export function startAuthFlow(cfg) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (url.pathname === '/callback') {
            ...
            if (error) {
                html(200, `<h1>Authorization error</h1><p>${error}</p>`);
                return;
            }
            if (state !== stateParam || !code) {
                html(400, '<h1>Invalid request</h1>');
                return;
            }
            try {
        ...
        if (url.pathname === '/save-config' && req.method === 'POST') {
            try {
                ...
                if (mutationTokenTtlSeconds)
                    ...
                    delete process.env['GOOGLE_ADS_MUTATION_TOKEN_TTL_SECONDS'];
                if (confirmStateTtlSeconds)
                    process.env['GOOGLE_ADS_CONFIRM_STATE_TTL_SECONDS'] = confirmStateTtlSeconds;
                else
                    delete process.env['GOOGLE_ADS_CONFIRM_STATE_TTL_SECONDS'];
