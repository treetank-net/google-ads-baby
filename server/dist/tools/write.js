...
import { createAdGroup, createAssetGroup, createAssetGroupAssets, createAssetGroupListingGroupFilters, createAssetGroupSignals, createCampaignTargeting, createDisplayAdGroup, createDisplayCampaign, createKeywords, createNegativeKeywords, createPerformanceMaxCampaign, createResponsiveDisplayAd, createResponsiveSearchAd, createSearchCampaign, executeGaql, mutateCampaignBudget, mutateCampaignStatus, removeCampaigns, uploadImageAssetFromFile, uploadImageAssetFromUrl, } from '../client.js';
...
export function registerWriteTools(server, cfg) {
    ...
    server.tool('prepare_asset_group', 'Prepare creation of a paused Performance Max asset group under an existing campaign. Returns a preview and confirmation token.', {
        ...
        const countByField = (field) => normalizedAssets.filter((asset) => asset.field_type === field).length;
        if (countByField('HEADLINE') < 3)
            return validationResult('Performance Max asset groups require at least 3 HEADLINE assets.');
        if (countByField('LONG_HEADLINE') < 1)
            return validationResult('Performance Max asset groups require at least 1 LONG_HEADLINE asset.');
        if (countByField('DESCRIPTION') < 2)
            return validationResult('Performance Max asset groups require at least 2 DESCRIPTION assets.');
        if (countByField('MARKETING_IMAGE') < 1)
            return validationResult('Performance Max asset groups require at least 1 MARKETING_IMAGE asset.');
        if (countByField('SQUARE_MARKETING_IMAGE') < 1)
            return validationResult('Performance Max asset groups require at least 1 SQUARE_MARKETING_IMAGE asset.');
    ...
    server.tool('prepare_asset_group_listing_groups', 'Prepare creation of Performance Max asset group listing group trees. Returns a preview and confirmation token.', {
        customer_id: z.string().describe('Google Ads customer ID from list_accounts'),
        asset_group_id: z.string().describe('Existing asset group ID'),
    ...
    server.tool('confirm_mutation', 'Execute a previously prepared mutation. Requires a valid, non-expired token from a prepare_* call. The user MUST have explicitly confirmed the action.', {
        ...
        try {
            ...
            if (mutation.action === 'asset_group_create') {
                const result = await createAssetGroup(cfg, p.customer_id, p.campaign_id, p.asset_group_name, p.final_urls, p.assets.map((asset) => ({
            ...
            if (mutation.action === 'asset_group_listing_group_filters_create') {
                const result = await createAssetGroupListingGroupFilters(cfg, p.customer_id, p.asset_group_id, p.nodes.map((node) => {
